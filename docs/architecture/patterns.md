...
Важно: документ описывает сразу два уровня архитектуры. На уровне кодовой базы проект вырос из `modular monolith` с четкими bounded context, а на уровне runtime и deployment уже развертывается как микросервисная топология с отдельными API- и worker-процессами. Поэтому паттерны ниже относятся либо к структуре кода, либо к текущей эксплуатационной модели, либо к обоим слоям сразу.

## Архитектурный уровень

### 1. Modular Monolith

Исторический базовый стиль проекта на уровне кодовой базы — модульный монолит. Это уже не описание всей эксплуатационной топологии целиком, а описание того, как организованы доменные границы, shared code и слои приложения внутри одного репозитория.
...
Зачем был выбран как стартовый паттерн:
...
Что изменилось после эволюции к микросервисам:

- bounded context и модульные границы сохранились;
- но runtime больше не обязан быть единым процессом;
- те же модули теперь разворачиваются как отдельные API-сервисы и worker-ы;
- поэтому этот паттерн нужно читать как основу структуры кода, а не как единственное описание текущего production deployment.
...
Ограниченные контексты используются независимо от того, работает ли система как единый процесс или как набор микросервисов. Это паттерн из Domain-Driven Design, который помогает не смешивать разные смыслы одних и тех же сущностей и как раз делает возможным безопасный split по сервисам.
...
#### Связь bounded context с runtime-топологией

Эта таблица показывает, что bounded context в проекте не остаются только логической группировкой кода. У каждого контекста уже есть вполне конкретное отображение в API-сервисы, worker-ы, Kafka-топики и read-model слой.

| Bounded context | Основной API-сервис | Основной worker / consumer | Kafka topics | Основные read models |
| --- | --- | --- | --- | --- |
| `registry` | `registry-api` | `projection-worker` | `registry.events`, `audit.events` | `agents`, `agent_versions`, `graph_definitions`, `model_endpoints`, `model_versions`, `tool_definitions`, `environments`, `deployments` |
| `orchestration` | `orchestration-api`, `orchestration-query-api` | `execution-worker`, `projection-worker` | `agent.executions`, `agent.steps`, `audit.events` | `execution_runs`, `execution_steps` |
| `monitoring` | `monitoring-api` | `analytics-worker`, `projection-worker` | `system.metrics`, `system.health`, `model.inference`, `cost.events`, `anomaly.events`, `drift.events` | `metric_samples`, `health_check_results`, `cost_records`, `anomaly_reports`, `drift_reports` |
| `alerting` | `alerting-api` | `alerts-worker`, `projection-worker` | `alerts.events`, `anomaly.events`, `drift.events`, `system.health` | `alerts` |
| `audit` | `audit-api` | `projection-worker` | `audit.events` | `audit_events` |

Что важно в этой карте:

- один bounded context может иметь и command-side, и query-side сервисы, как это уже сделано для `orchestration`;
- один и тот же Kafka topic может быть полезен сразу нескольким контекстам, но ownership business-смысла при этом остается у исходного bounded context;
- read models физически живут в общей PostgreSQL-схеме, но логически принадлежат своим контекстам и обслуживаются через отдельные query API.
...
### 27. Deployment Profile Override

На эксплуатационном уровне проект использует отдельный паттерн профилирования deployment-конфигурации: глобальный baseline описывается один раз, а конкретный API-сервис или worker переопределяет только те параметры, которые действительно отличаются.

Где используется:

- [helm/workflow-platform/values.yaml](d:/p/FastAPI/FastAPI_Layers/helm/workflow-platform/values.yaml)
- [helm/workflow-platform/templates/deployment-api.yaml](d:/p/FastAPI/FastAPI_Layers/helm/workflow-platform/templates/deployment-api.yaml)
- [helm/workflow-platform/templates/deployment-workers.yaml](d:/p/FastAPI/FastAPI_Layers/helm/workflow-platform/templates/deployment-workers.yaml)
- [helm/workflow-platform/templates/hpa.yaml](d:/p/FastAPI/FastAPI_Layers/helm/workflow-platform/templates/hpa.yaml)

Как это работает:

- глобальный профиль для API живет в `api.*`, для worker-ов в `workers.defaults.*`;
- локальные overrides живут в `apiServices[]` и `workers[]`;
- effective-конфиг собирается в Helm-шаблонах через `default`, `mergeOverwrite`, `deepCopy` и `concat`;
- если override пустой, роль наследует общий baseline, а не требует копирования всего блока.

Зачем выбран:

- `gateway`, `orchestration-query-api`, `execution-worker` и остальные процессы имеют разный характер нагрузки и разные требования к rollout;
- chart остается production-ready, но при этом не разрастается в набор полностью дублирующих values-профилей;
- команде проще обсуждать и ревьюить именно различия между ролями, а не повторяющиеся YAML-блоки.

Trade-off:

- Helm-шаблоны становятся сложнее;
- любые ошибки merge-логики лучше обязательно проверять через `helm template` или equivalent validation.

### 28. Compatibility Gateway

После перехода к микросервисной topology в проекте сохранен отдельный compatibility gateway как переходный паттерн.

Где используется:

- [app/main.py](d:/p/FastAPI/FastAPI_Layers/app/main.py)
- [app/api/router.py](d:/p/FastAPI/FastAPI_Layers/app/api/router.py)
- [docker-compose.yml](d:/p/FastAPI/FastAPI_Layers/docker-compose.yml)
- [helm/workflow-platform/templates/ingress.yaml](d:/p/FastAPI/FastAPI_Layers/helm/workflow-platform/templates/ingress.yaml)

Как это работает:

- отдельные bounded context API уже живут как самостоятельные сервисы;
- `gateway-api` агрегирует их маршруты и сохраняет единый внешний вход;
- ingress по умолчанию публикует именно gateway, а остальные сервисы открываются адресно и поэтапно.

Зачем выбран:

- позволяет перейти к микросервисам без одномоментной ломки существующих клиентов и локальных сценариев;
- снижает риск во время миграции deployment и observability-контуров;
- дает время постепенно отделять внутренние read/admin API от внешнего публичного входа.

Trade-off:

- gateway сохраняет часть связности и дублирует часть API surface;
- если оставить его навсегда без дальнейшего упрощения, он может превратиться в новый центр связанности.
...
9. `Deployment Profile Override` и `Compatibility Gateway` помогают пройти путь от общей модульной кодовой базы к микросервисной эксплуатации без резкого разрыва контрактов.
...
| Deployment Profile Override | Позволяет централизованно держать baseline и точечно тюнить роли под разную нагрузку | Усложняет Helm-шаблоны и требует аккуратной валидации merge-логики | Средняя |
| Compatibility Gateway | Позволяет мигрировать к микросервисам без одномоментной ломки внешних контрактов | Может затянуть переходный период и сохранить лишнюю связанность | Низкая |
...
| Saga / Process Manager | Межмодульная координация в основном строится на независимых consumers и eventual consistency | Пока нет длинных бизнес-транзакций через несколько независимых сервисов с компенсациями. В текущей архитектуре это еще решается проще на уровне событийных consumers и сервисных границ | Когда orchestration, billing, deployment и внешние интеграции будут разнесены по разным сервисам и появятся распределенные rollback-сценарии | рост числа компенсационных операций, появление “полусобранных” процессов, необходимость формализованной оркестрации между сервисами |
...
| Важны для масштабирования | `Modular Monolith`, `Bounded Context`, `Ports and Adapters`, `Gateway / Adapter`, `Workflow / Pipeline`, `Upsert Projection`, `Fallback`, `Deployment Profile Override`, `Compatibility Gateway` | Эти решения позволяют отделять зоны ответственности, выносить части системы в отдельные рантаймы, масштабировать обработку событий, постепенно менять topology deployment-а и снижать связанность между модулями и внешними интеграциями. Они особенно полезны, когда растет число команд, окружений и сценариев нагрузки. |
...
- Если обсуждается рост нагрузки, дальнейшее дробление сервисов или увеличение числа интеграций, ориентируйтесь на строку `Важны для масштабирования`.
