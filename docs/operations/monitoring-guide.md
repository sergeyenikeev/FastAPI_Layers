...
Если `orchestration-query-api` публикуется отдельным ingress, scrape-конфигурация не меняется: Prometheus по-прежнему читает `/metrics` через внутренний `Service`, а не через внешний ingress. Это важно, потому что observability остается независимой от того, публикуется ли сервис наружу, на каком hostname он опубликован и какие ingress-аннотации для него заданы.
