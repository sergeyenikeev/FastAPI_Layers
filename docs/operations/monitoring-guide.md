...
- Prometheus скрапит `api:8080/metrics` по конфигу из `docker/prometheus.yml`.
...
Его конфигурация находится в `docker/otel-collector-config.yaml`.
Его конфигурация находится в `docker/otel-collector-config.yaml`.

Сейчас локальный collector:

- принимает OTLP по gRPC и HTTP;
- выводит traces в `logging` exporter.
