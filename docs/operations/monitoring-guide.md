...
- в collector и downstream backend трассы уже различаются как `gateway-api`, `registry-api`, `orchestration-api` и т.д.

За счет этого можно:

- отличать деградацию `registry-api` от деградации `orchestration-api`;
